#!/bin/bash
# install-rcforge.sh - Installs the rcForge system with security enhancements
# Securely installs rcForge with appropriate permissions
...
# Check if running as root (before any other code)
check_root() {
...
echo -e "${BLUE}┌──────────────────────────────────────────────────────┐${RESET}"
echo -e "${BLUE}│ rcForge Installation                                 │${RESET}"
echo -e "${BLUE}└──────────────────────────────────────────────────────┘${RESET}"
echo ""
...
# Create rcForge directory structure with secure permissions
mkdir -p "$RCFORGE_DIR"
chmod 700 "$RCFORGE_DIR"  # User can read/write/execute, no permissions for group/world
mkdir -p "$RCFORGE_DIR/scripts"
chmod 700 "$RCFORGE_DIR/scripts"
mkdir -p "$RCFORGE_DIR/checksums"
chmod 700 "$RCFORGE_DIR/checksums"
...
echo -e "${GREEN}✓ Created directory structure with secure permissions${RESET}"
...
if [[ -f "$SOURCE_DIR/rcforge.sh" ]]; then
  ...
  chmod 700 "$RCFORGE_DIR/rcforge.sh"  # Make executable for user only
...
if [[ -f "$SOURCE_DIR/include-structure.sh" ]]; then
  ...
  chmod 700 "$RCFORGE_DIR/include-structure.sh"  # Make executable for user only
...
if [[ -d "$SOURCE_DIR/docs" ]]; then
  ...
  # Ensure documentation files are readable only by user
  find "$RCFORGE_DIR/docs" -type f -exec chmod 600 {} \;
  echo -e "${GREEN}✓ Installed documentation${RESET}"
fi

# Make sure all scripts are executable (for user only)
find "$RCFORGE_DIR" -name "*.sh" -type f -exec chmod 700 {} \;
echo -e "${GREEN}✓ Set secure permissions on all scripts${RESET}"
...
if [[ ! -d "$RCFORGE_DIR/include/path" && "$MINIMAL" -ne 1 ]]; then
  ...
  if [[ -f "$RCFORGE_DIR/include-structure.sh" ]]; then
    ...
    # Ensure directories have secure permissions
    find "$RCFORGE_DIR/include" -type d -exec chmod 700 {} \;
  fi
fi
...
EOF
  chmod 700 "$RCFORGE_DIR/scripts/100_global_common_environment.sh"
...
EOF
  chmod 700 "$RCFORGE_DIR/scripts/200_global_common_aliases.sh"
...
EOF
  chmod 700 "$RCFORGE_DIR/scripts/050_global_common_path.sh"
...
PS2="${GRAY}→ ${RESET}"
EOF
    chmod 700 "$RCFORGE_DIR/scripts/300_global_bash_prompt.sh"
...
RPROMPT=''
EOF
    chmod 700 "$RCFORGE_DIR/scripts/500_global_zsh_prompt.sh"
...
EOF
  chmod 700 "$RCFORGE_DIR/scripts/500_${CURRENT_HOSTNAME}_common_settings.sh"
...
All files and directories are secured with restrictive permissions (readable/writable
only by your user). This is important because shell configs often contain sensitive
information like API keys, tokens, or server addresses.

The installer and other utilities refuse to run as root to prevent permission issues.
EOF
chmod 600 "$RCFORGE_DIR/README.md"
...
update_rc_files() {
  ...
  if [[ ! -f "$rc_file" ]]; then
    ...
    touch "$rc_file"
    chmod 600 "$rc_file"  # Secure permissions for shell RC file
  fi
...
EOF
  ...
  # Ensure RC file has correct permissions
  chmod 600 "$rc_file"
}
...
echo -e "\n${GREEN}┌──────────────────────────────────────────────────────┐${RESET}"
echo -e "${GREEN}│ rcForge Installation Complete                         │${RESET}"
echo -e "${GREEN}└──────────────────────────────────────────────────────┘${RESET}"
echo ""
